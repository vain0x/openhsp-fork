...
private:
	std::shared_ptr<HspCompilerApi> api_;
	...
	//最後に開かれた compiler
	//マルチスレッド化するときには仕様を考えなおすべし
	static std::stack<std::shared_ptr<HspCompilerApi>> hspcmp_stack;
public:
	static std::shared_ptr<HspCompilerApi> lastHspcmp();
};
