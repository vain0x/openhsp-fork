...
^
モジュール変数名は、モジュールに関連付けられたローカルな変数を登録しておくものです。登録されたモジュール変数名は、#modfuncで定義された処理ルーチン内で使用することができるようになります。
^
...
#deffunc命令で定義した位置より以降は、指定された名前を命令語として使用することが可能です。
^
...
#defcfunc命令で定義した位置より以降は、 指定された名前を関数として使用することが可能です。
^
