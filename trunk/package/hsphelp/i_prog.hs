...
*label : ラベル名
%inst
valで指定された変数に代入されている値と、 p2の比較値により指定したラベルに条件でジャンプします。
比較方法は、p1の比較フラグが0以上(正値)の場合は(valの値>=p2)が成立した時にジャンプ、p1が-1以下(負値)の場合は (valの値<=p2)が成立した時にジャンプを行ないます。この命令は、 forなど一部のマクロ処理を高速化するために用意された命令です。
